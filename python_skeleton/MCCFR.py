...
class CFR_Game_State:
    ...
    def is_terminal(self,history):
        ...
        if(history[-3:] == 'FLD'):
            return True
        elif(len(self.board_cards)==5): #we only allow 2 actions per street, and we fully solve preflop using precomputed range charts
            return True
        else:
            return False
    @staticmethod
    ...
    def apply_action(self, history, action_string, player_num):
       ...
       if(action_string[0]=='R'):#raise
           ...
           self.cur_street.append(action_string)
    ...
    def get_all_actions(self, history, player_num):
        ...
        Gets all the current actions that we can take from our state
        '''
        default_actions = ['R66', 'R75', 'R80', 'RCC', 'CAL', 'CHK', 'FLD']
        last_action = '' if len(history) < 3 else history[-3:]
        print("THE CURRENT STREET: {}".format(self.cur_street))

        if(len(self.cur_street) == CFR_Game_State.MAX_ACTIONS_PER_STREET):
            self.board_cards += [self.deck[0]]
            del self.deck[0]
            print("UPDATE BOARD")
            if(last_action[0]=='R'):
                self.cur_street.clear()
                return ['CAL','FLD']
            else:
                self.cur_street.clear()
                return ['CHK', 'FLD']
            ...
            for i in range(len(default_actions)-1,-1,-1):
                if(last_action==''):
                    ...
                    if(default_actions[i][0]=='R'):
                        sizing = 2 if default_actions[i][1:] == 'CC' else float(default_actions[i][1:]) / 100
                        betsize = sizing * self.pot_size
                        our_stack = self.p1_stack if player_num == 0 else self.p2_stack
                        if (betsize > our_stack):
                            del default_actions[i]
                else:
                    if(last_action[0]=='R'):
                        if(default_actions[i]=='CHK'):
                            del default_actions[i]
                        if(default_actions[i]=='CAL'):
                            sizing = 2 if last_action[1:]=='CC' else float(last_action[1:])/100
                            betsize = (float(sizing)/(sizing+1))*self.pot_size
                            our_stack = self.p1_stack if player_num==0 else self.p2_stack
                            if(betsize > our_stack):
                                del default_actions[i]
                        elif(default_actions[i][0]=='R' and len(self.cur_street)<=2):#force the bot to not spam reraise (although it still is able to)
                            sizing = 2 if default_actions[i][1:]=='CC' else float(default_actions[i][1:])/100
                            betsize = sizing * self.pot_size
                            our_stack = self.p1_stack if player_num==0 else self.p2_stack
                            if(betsize > our_stack):
                        ...
                        if(default_actions[i]=='FLD' or default_actions[i] == 'CAL'):
                            del default_actions[i]
                        elif(default_actions[i][0]=='R'):
                            sizing = 2 if default_actions[i][1:] == 'CC' else float(default_actions[i][1:]) / 100
                            betsize = sizing * self.pot_size
                            our_stack = self.p1_stack if player_num == 0 else self.p2_stack
                            if (betsize > our_stack):
                        ...
                        if(default_actions[i]=='CAL'):
                            del default_actions[i]
                        if (default_actions[i][0] == 'R' and len(self.cur_street) <= 1):
                            sizing = 2 if default_actions[i][1:] == 'CC' else float(default_actions[i][1:]) / 100
                            betsize = sizing * self.pot_size
                            our_stack = self.p1_stack if player_num == 0 else self.p2_stack
                            if (betsize > our_stack):
...
class MCCFR():
    ...
    def mc_cfrm(self, game_state : CFR_Game_State, history_state, reach_probabilities, cur_player):
        ...
        do_not_raise = False
        if (len(cur_game_state.cur_street) == 2 and cur_game_state.cur_street[0][0] == 'R' and
                cur_game_state.cur_street[1][0] == 'R'):
            do_not_raise = True
            cur_game_state.cur_street.clear()
        ...
        if (cur_game_state.is_terminal(history_state)):
            ...
            print("POT SIZE: {}".format(cur_game_state.pot_size))
            print("STACK SIZES: {}, {}".format(cur_game_state.p1_stack,cur_game_state.p2_stack))
            return utility
        ...
        print("HISTORY STATE: {}".format(history_state))
        print("BOARD STATE: {}".format(cur_game_state.board_cards))
        print("CURRENT STREET: {}".format(cur_game_state.cur_street))
        print('------------------------------------------------------------------------------------------')
        ...
        string_to_index = {}
        string_to_index['R66']=0
        string_to_index['R75']=1
        string_to_index['R80']=2
        string_to_index['RCC']=3
        string_to_index['CAL']=4
        string_to_index['CHK']=5
        string_to_index['FLD']=6


        for val in (all_actions):
            if(do_not_raise):
                if(val[0]=='R'):
                    continue
            action_prob = cur_strategy[string_to_index[val]]
            nxt_reach = reach_probabilities.copy()
            nxt_reach[cur_player] *= action_prob
            history_state += val
            cfr_vals[string_to_index[val]] = -self.mc_cfrm(cur_game_state, history_state, nxt_reach, (cur_player + 1) % 2)
...
#print(cfr_test_run.get_all_actions('CAL',0))

'''
print(cfr_test_run.board_cards)
cfr_test_run.get_all_actions('',1)
...
print("----------------------------------------------------------")
print(cfr_test_run.board_cards)
cfr_test_run.apply_action('RCCRCC', 'CAL', 1)
print(cfr_test_run.get_all_actions('RCCRCCCAL',0))
print(cfr_test_run.board_cards)
